    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Person` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `name` TEXT NOT NULL, `pointer` INTEGER NOT NULL)');
...
class _$PersonDao extends PersonDao {
  ...
  Future<List<Person>> findAllPeople() async {
    return _queryAdapter.queryList('SELECT * FROM Person',
        mapper: (Map<String, Object?> row) => Person(
            ...
            pointer: row['pointer'] as int));
  }

  @override
  Future<bool?> deletePerson(int id) async {
    return _queryAdapter.query('DELETE FROM Person WHERE id = ?1',
  ...
  Future<void> updateUser(Person person) async {
    await _personUpdateAdapter.update(person, OnConflictStrategy.ignore);
  }
}
